...
# -----------------------------
    ...
    if st.session_state.cashier != "admin":
        ...
        tab1, tab2, tab3 = st.tabs(["➕ Add Cashier", "📋 Cashier List", "⚙️ Status Management"])
            ...
            try:
                ...
                if cashiers:
                    for cashier in cashiers:
                        ...
                        with col1:
                            st.write(f"👤 {cashier['username']} — Status: {'🟢 Active' if cashier['active'] else '🔴 Inactive'}")
                        with col2:
                            ...
                            if st.button(btn_label, key=f"status_{cashier['id']}"):
                                try:
                                    ...
                                    st.success(f"✅ {cashier['username']} set to {'Active' if new_status else 'Inactive'}")
                                    st.experimental_rerun()
                                except Exception as e:
